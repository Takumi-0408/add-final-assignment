...
- [ ] **1-1** `useCurrentLocation` フックの実装と単体テスト
...
- [ ] **1-3** オンボーディング画面（S-02）の実装
  - 初回起動判定（AsyncStorage）
  - 位置情報許可のリクエスト導線
- [ ] **1-4** スプラッシュ画面（S-01）の実装
...
- [ ] **2-1** Firebase Auth サービス層の実装と単体テスト（`src/services/firebase/auth.ts`）
  - 匿名認証・メール/パスワード認証・Google 認証
  - エラーコードを `AppError` に変換するロジック
- [ ] **2-2** 認証状態を管理する Zustand ストアの実装とテスト（`src/stores/authStore.ts`）
- [ ] **2-3** ログイン画面（S-03）の実装と UI テスト
- [ ] **2-4** サインアップ画面（S-04）の実装と UI テスト
- [ ] **2-5** 匿名→本登録のアカウントリンク処理の実装とテスト
